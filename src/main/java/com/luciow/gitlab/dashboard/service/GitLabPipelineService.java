...
public class GitLabPipelineService {
    ...
    public Single<Build> getLatestBuild(int projectId, String ref, String buildName, String stage) {
        Pipeline pipeline = getLatestPipeline(projectId, ref).toBlocking().value();

        log.info("Found {} pipeline", pipeline.getId());

        return Observable.range(1, Integer.MAX_VALUE)
                .concatMap(
                        page -> Observable.defer(() -> Observable.just(gitLabApiFacade.getBuilds(projectId, pipeline.getSha(), page, pageSize)))
    ...
    private Single<Pipeline> getLatestPipeline(int projectId, String ref) {
        return Observable.range(1, Integer.MAX_VALUE)
                .concatMap(
                        page -> Observable.defer(() -> Observable.just(gitLabApiFacade.getPipelines(projectId, page, pageSize)))
                )
                .takeWhile(page -> !page.isEmpty())
                .concatMap(Observable::from)
