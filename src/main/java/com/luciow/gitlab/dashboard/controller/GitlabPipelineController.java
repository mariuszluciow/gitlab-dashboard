...
public class GitlabPipelineController {
    ...
    public Single<String> getLatestBuildStatus(@PathVariable("projectId") int projectId,
        ...
        return gitLabPipelineService.getLatestBuild(projectId, ref, buildName, stage)
                .map(Build::getStatus);
    }
    ...
    public byte[] getAvatar(@PathVariable("projectId") int projectId,
                            ...
                            @PathVariable("buildName") String buildName,
                            @PathVariable("stage") String stage) {
        String avatarUri = gitLabPipelineService.getLatestBuild(projectId, ref, buildName, stage)
    ...
    @RequestMapping(value = "/pipeline/{projectId}/{ref}/{stage}/{buildName}/status.svg", produces = "image/svg+xml;charset=utf-8")
    public String getPic(@PathVariable("projectId") int projectId,
                         @PathVariable("ref") String ref,
                         @PathVariable("buildName") String buildName,
                         @PathVariable("stage") String stage) {
        Build build = gitLabPipelineService.getLatestBuild(projectId, ref, buildName, stage).toBlocking().value();
